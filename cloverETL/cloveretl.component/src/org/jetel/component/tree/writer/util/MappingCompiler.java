	...
	private PortBinding compilePortBinding(ContainerNode container, Tag tag) throws ComponentNotReadyException {
		...
		if (stringKeysList != null) {
			...
			for (int i = 0; i < keys.length; i++) {
				keys[i] = metadata[portIndex].getFieldPosition(stringKeysList.get(i));
			}
		}
		int[] parentKeys = null;
		...
		if (filterExpression != null) {
			recordFilter = RecordFilterFactory.createFilter(FILTER_PREFIX + filterExpression, metadata, graph, componentId, null, null, logger);
		}
