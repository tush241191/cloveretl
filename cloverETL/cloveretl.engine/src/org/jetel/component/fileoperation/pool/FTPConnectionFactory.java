...
public class FTPConnectionFactory implements ConnectionFactory {
	...
	@Override
	public PoolableConnection makeObject(Authority key) throws Exception {
		PooledFTPConnection connection = new PooledFTPConnection(key);
		connection.connect();
		return connection;
	}
	...
	public void destroyObject(Authority key, PoolableConnection obj)
			throws Exception {
		((PooledFTPConnection) obj).disconnect();
	}
	...
	public boolean supports(Authority authority) {
		return authority.getProtocol().equalsIgnoreCase(FTP_SCHEME);
	}
